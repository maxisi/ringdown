...
class Signal(TimeSeries):
    ...
    def project(
        ...
        if fd_shift:
            ...
            if interpolate:
                hint = np.interp1d(
                    self.time, h, kind="cubic", fill_value=0, bounds_error=False
                )
...
def get_detector_signals(
    times: dict | np.ndarray | None = None,
    ifos: list | None = None,
    ...
    **kws :
    ...
    p_kws = {
        k: s_kws.pop(k)
        for k in kws.keys()
        if k in getfullargspec(Signal.project)[0][1:] and k != "t0"
    }
    ...
    if all([i in trigger_times for i in ifos]):
        ...
        if t0 is None:
            ...
            trigger_times = {
                i: t0 + get_delay(i, t0, p_kws["ra"], p_kws["dec"])
                for i in ifos
            }
    else:
    ...
    if fast_projection:
        ...
        for i in ifos:
            sdict[i] = h.project(
                antenna_patterns=antenna_patterns.get(i),
