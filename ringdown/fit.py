...
def get_sampling_kwargs(**kwargs):
    ...
    run_kws = {
        ...
        if k not in SAMPLER_ARGS and k not in KERNEL_ARGS
    }
...
class Fit(object):
        ...
        for ifo in self.ifos:
            if not np.isclose(self.acfs[ifo].delta_t, self.data[ifo].delta_t):
                ...
                raise AssertionError(
                    e.format(
                        ifo, self.acfs[ifo].delta_t, self.data[ifo].delta_t
                    )
                )
    ...
    def cholesky_factors(self):
        ...
        return {
            i: a.iloc[: self.n_analyze].cholesky for i, a in self.acfs.items()
        }
        ...
        if config.has_section("injection"):
            ...
            if "path" in inj_kws:
                ...
                try:
                    ...
                    if overlap:
                        logger.warning(
                            "overwriting injection file options "
    ...
    def condition_data(
        ...
        settings = {
            k: v for k, v in locals().items() if k not in ["self", "warn"]
        }
    ...
    def load_data(
        ...
        **kws,
        ...
        for ifo, path in path_dict.items():
            self.add_data(
                Data.load(path, ifo=ifo, channel=channel_dict[ifo], **kws)
            )
    ...
    def from_imr_result(
        ...
        **kws,
        ...
        if condition:
            logger.info(
                "conditioning data based on IMR result: "
        ...
        if update_model:
            ...
            opts = imr.estimate_ringdown_prior(
                modes=fit.modes, cache=True, **prior_kws
            )
...
class FitSequence(Fit):
    ...
    @classmethod
    def from_config(
        cls, config_input: str | dict, no_cond: bool = False, **kws
    ):
