...
DEF_RUN_KWS = dict(dense_mass=True, num_warmup=1000, num_samples=1000, num_chains=4)
...
def get_sampling_kwargs(**kwargs):
    ...
    run_kws = {
        k: v for k, v in kws.items() if k not in SAMPLER_ARGS and k not in KERNEL_ARGS
    }
...
class Fit(object):
    ...
    def __init__(self, modes=None, strain_scale="auto", imr_result=None, **kws):
        self.info = {}
        self.data = {}
        self.injections = {}
        self.acfs = {}
    ...
    def run_input(self) -> list:
        ...
        for ifo in self.ifos:
            if not np.isclose(self.acfs[ifo].delta_t, self.data[ifo].delta_t):
                ...
                raise AssertionError(
                    e.format(ifo, self.acfs[ifo].delta_t, self.data[ifo].delta_t)
                )
        ...
        times = [np.array(d.time) - self.start_times[i] for i, d in data_dict.items()]
    ...
    def cholesky_factors(self):
        ...
        return {i: a.iloc[: self.n_analyze].cholesky for i, a in self.acfs.items()}
    ...
    def from_config(
        ...
        if "name" in model_opts:
            warnings.warn("model name is deprecated, use explicitmode options instead")
            logger.info("trying to guess mode configuration based on model name")
        ...
        if config.get(IMR_CONFIG_SECTION, "initialize_fit", fallback=False):
            ...
            if "data" in config:
                ...
                data_kws = {k: utils.try_parse(v) for k, v in config["data"].items()}
                if "ifos" in config["data"]:
                    data_kws["ifos"] = utils.get_ifo_list(config, "data")
            else:
                data_kws = {}
            logger.info("loading IMR result")
            fit = cls.from_imr_result(imr_path, **imr, **model_opts, data_kws=data_kws)
        else:
        ...
        if config.has_section(IMR_CONFIG_SECTION) and not fit.has_imr_result:
            ...
            if "path" not in imr or "imr_result" not in imr:
                ...
                logger.warning(
                    "no path to IMR result provided; ignoring  IMR section in config"
                )
        ...
        if "data" in config and not fit.has_data:
            ...
            kws = {
                k: utils.try_parse(v) for k, v in config["data"].items() if k != "ifos"
            }
        ...
        if config.has_section("injection"):
            ...
            if "path" in inj_kws:
                ...
                try:
                    ...
                    if overlap:
                        logger.warning(
                            f"overwriting injection file options with config: {overlap}"
                        )
        ...
        if result:
            if isinstance(result, az.InferenceData):
                ...
                logger.warning(
                    "loading result from disk with no guarantee of fit correspondence!"
                )
                try:
    ...
    def to_config(self, path=None):
        ...
        # prior options
        config["model"].update(
            {k: utils.form_opt(v, key=k) for k, v in self.model_settings.items()}
        )
    ...
    def condition_data(
        self, preserve_acfs: bool = False, silent: bool = False, **kwargs
    ):
        ...
        settings = {k: v for k, v in locals().items() if k not in ["self", "warn"]}
    ...
    def get_templates(self, signal_buffer="auto", **kws):
        ...
        **kws :
        ...
        if not all([k in kws for k in sky_keys]):
            kws["antenna_patterns"] = (
                kws.pop("antenna_patterns", None) or self.antenna_patterns
            )
        for k in sky_keys:
    ...
    def conditioned_injections(self) -> dict:
        ...
        if self.injections:
            if "condition" in self.info:
                ...
                hdict = {
                    i: h.condition(t0=self.start_times[i], **c)
                    for i, h in self.injections.items()
                }
            else:
    ...
    def add_data(self, data, time=None, ifo=None, acf=None):
        ...
        if self.has_target:
            try:
                ...
                logger.warning(
                    "data incompatible with target! removing target (please reset)"
                )
                self.target = None
                    ...
                    if freq is None:
                        if f_max is None:
                            if delta_t is not None:
                                ...
                                raise ValueError(
                                    "provide freq, f_max, f_samp or delta_t"
                                )
                        if delta_f is None:
                            if duration is None:
                                raise ValueError("provide duration or delta_f")
                            delta_f = 1 / duration
                    psd = PowerSpectrum.from_lalsimulation(
                        p,
                        freq=freq,
                        f_min=f_min,
                        f_max=f_max,
    ...
    def load_acfs(
        self, path=None, ifos=None, from_psd=False, from_imr_result=False, **kws
    ):
        ...
        if from_imr_result:
            if self.imr_result is None:
                raise ValueError(
                    "no IMR result available; load using Fit.load_imr_result"
                )
    ...
    def duration(self) -> float:
        ...
        if self._n_analyze and not self.target.duration:
            if self.data:
                ...
                logger.warning(
                    "add data to compute duration (n_analyze = {})".format(
                        self._n_analyze
                    )
                )
                return None
    ...
    def add_imr_result(
        self, imr_result: pd.DataFrame | dict | np.ndarray, **kws
    ) -> None:
        ...
        **kws,
        ...
        if set_target:
            ...
            if advance_target_by_mass:
                ...
                logger.info(
                    f"advancing target time by {dt} s "
        ...
        if condition:
            logger.info(
                f"conditioning data based on IMR result: {imr.condition_options}"
            )
        ...
        if load_acfs:
            ...
            if not condition:
                logger.warning("ACFs derived from IMR result but data not conditioned!")

        if update_model:
            prior_kws = prior_kws or {}
            prior_kws["prng"] = prior_kws.get("prng", prng)
            opts = imr.estimate_ringdown_prior(modes=fit.modes, cache=True, **prior_kws)
...
class FitSequence(Fit):
    ...
    def __repr__(self):
        return (
            f"FitSequence(modes={self.modes}, ifos={self.ifos}, "
    ...
    def run(
        ...
        **kwargs :
        ...
        r = []
        sampler = None
        for t0, target in tqdm(
            self.target_collection.items(),
            desc="targets",
    ...
    @classmethod
    def from_config(cls, config_input: str | dict, no_cond: bool = False, **kws):
