...
INT_INDICES = [1, 2, 3]
ALL_INDICES = TUPLE_INDICES + STR_INDICES_LONG + STR_INDICES_SHORT + INT_INDICES
...
@pytest.mark.parametrize(["index", "ref_index"],
                         zip(STR_INDICES_LONG + STR_INDICES_SHORT, TUPLE_INDICES))
...
def test_mode_index_list(indices):
    ...
    assert all(isinstance(m, ringdown.indexing.ModeIndex) for m in mil.indices)